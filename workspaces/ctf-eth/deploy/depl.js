...
const VerifyingPaymasterArtifact = require( '@opengsn/paymasters/build/contracts/VerifyingPaymaster')
module.exports = async function ({getChainId, getNamedAccounts, ethers, deployments}) {
  ...
  if (process.env.DEPLOY_PM || chainId == 1337 || chainId == 31337) {
    ...
    ret = await deploy('VerifyingPaymaster', {contract: VerifyingPaymasterArtifact, gasPrice, from: deployer})
    const pm = await new ethers.Contract(ret.address, ret.abi, signer, {gasPrice})
    ...
    if (await pm.getHubAddr() != hub) {
      console.log('setting relayhub to: ', hub)
      await pm.setRelayHub(hub)
    }
    //the default captcha service signers
    const verifySigner = '0x2e988a386a799f506693793c6a5af6b54dfaabfb'
    if (await pm.functions.signer() == constants.ZERO_ADDRESS) {
      console.log('setting signer to: ', verifySigner)
      await pm.setSigner(verifySigner)
    } else {
      console.log('current pm signer=', await pm.functions.signer())
    }
